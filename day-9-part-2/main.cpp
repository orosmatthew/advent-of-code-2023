...
static int extrapolate_front(const std::array<int, 64>& data, const int size) // NOLINT(*-no-recursion)
{
    ...
    return data[0] - extrapolate_front(next, next_size);
}
...
static int solve(const std::string& data)
{
    ...
    while (i < data.size()) {
        ...
        if (data[i] == '\n') {
            total += extrapolate_front(nums, nums_size);
            nums_size = 0;
        }
        i++;
    }
...
int main()
{
    const std::string data = read_data("./day-9-part-2/input.txt");
